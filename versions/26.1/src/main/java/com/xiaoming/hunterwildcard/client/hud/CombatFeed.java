...
public final class CombatFeed {
    ...
    public static void kill(String hunter,String runner,int current,int target,boolean environment){
        ...
        sm.play(SimpleSoundInstance.forUI(SoundEvents.ARROW_HIT_PLAYER,environment?0.75F:1.0F));
    }
    public static void notice(String title,String a,String b,String style){
        if ("kill".equals(style) && a != null && a.startsWith(key("hud.feedback.versus")+HunterWildcardText.SPEC_SEPARATOR)) {
            ...
            if(parts.length==3){Entry e=new Entry(title,parts[1],parts[2],"kill",90,0,0);e.status=b;e.environment=title.contains("env_kill");enqueue(e);Minecraft.getInstance().getSoundManager().play(SimpleSoundInstance.forUI(SoundEvents.ARROW_HIT_PLAYER,e.environment?0.75F:1F));return;}
        }
    ...
    public static void render(GuiGraphicsExtractor c,int reservedTop){
        var mc=Minecraft.getInstance();if(mc.player==null||mc.options.hideGui||entries.isEmpty())return;
        ...
        int sw=c.guiWidth(),sh=c.guiHeight();
        ...
        if(mc.level!=null && mc.level.getScoreboard().getDisplayObjective(net.minecraft.world.scores.DisplaySlot.SIDEBAR)!=null) bottom=Math.min(bottom,sh/2-8);
        ...
        for(int i=0;i<Math.min(count,entries.size());i++){
            ...
            var m=c.pose();m.pushMatrix();m.translate(x,y);m.scale(scale,scale);
            ...
            c.item((e.style.equals("kill")?Items.IRON_SWORD:Items.PAPER).getDefaultInstance(),9,4);
            ...
            if(e.target>=0){
                ...
                var lines=mc.font.split(Component.literal(tr(e.a)),width-20);
                for(int j=0;j<Math.min(2,lines.size());j++)c.text(mc.font,lines.get(j),10,22+j*11,UiTheme.TEXT,false);
        ...
        if(count>0 && entries.size()>count) c.text(mc.font,Component.translatable("hunterwildcard.ui.feed.queued",entries.size()-count),sw-prefs.inset-92,top+count*(Math.round(height*scale)+6),UiTheme.TEXT,true);
    }
    private static void text(GuiGraphicsExtractor c,String value,int x,int y,int width,int color){
        var font=Minecraft.getInstance().font;
        if(font.width(value)>width)value=font.plainSubstrByWidth(value,Math.max(1,width-font.width("…")))+"…";
        c.text(font,Component.literal(value),x,y,color,false);
    }
