...
public final class GameStatusHud {
    ...
    public static void render(GuiGraphicsExtractor context, int topY) {
        Minecraft client = Minecraft.getInstance();
        SyncConfigPayload sync = ClientGameStatus.latest();
        if (sync == null || client.options.hideGui) {
            return;
        }
        ...
        int valueWidth = 0;
        for (Row row : rows) {
            labelWidth = Math.max(labelWidth, textRenderer.width(row.label));
            valueWidth = Math.max(valueWidth, textRenderer.width(row.value));
        }
        ...
        int neededWidth = 6 + labelWidth + 4 + Math.max(valueWidth, textRenderer.width(hint)) + 6;
        ...
        context.text(textRenderer, Component.literal(trim(textRenderer, title, panelWidth - 10)), panelX + 6, panelY + 4, accent, false);
        ...
        for (int i = 0; i < rows.size(); i++) {
            ...
            context.text(textRenderer, Component.literal(trim(textRenderer, row.label, labelWidth)), panelX + 6, rowY, COLOR_LABEL, false);
            context.text(textRenderer, Component.literal(trim(textRenderer, row.value, valueAreaWidth)), valueX, rowY, row.color, false);
        }
        context.text(textRenderer, Component.literal(trim(textRenderer, hint, panelWidth - 10)), panelX + 6, panelY + panelHeight - 10, COLOR_HINT, false);
    }
    ...
    private static String trim(Font textRenderer, String text, int maxWidth) {
        ...
        return textRenderer.plainSubstrByWidth(text, Math.max(0, maxWidth - textRenderer.width("..."))) + "...";
    }
