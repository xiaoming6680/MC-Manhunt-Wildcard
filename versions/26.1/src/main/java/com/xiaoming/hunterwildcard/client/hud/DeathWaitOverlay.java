...
public final class DeathWaitOverlay {
    ...
    public static void register() {
        HudElementRegistry.addLast(Identifier.fromNamespaceAndPath(HunterWildcardMod.MOD_ID, "death_wait_cover"), (context, tickCounter) -> render(context));
    }
    ...
    private static void render(GuiGraphicsExtractor context) {
        ...
        if (client.player == null || client.player.isDeadOrDying() || client.options.hideGui) return;
        ...
        int width = Math.min(310, context.guiWidth() - 24);
        int x = (context.guiWidth() - width) / 2;
        int height = visible ? 76 : 50;
        int y = Math.max(10, context.guiHeight() - height - 30);
    ...
    private static void centered(GuiGraphicsExtractor context, Font font, String text, int x, int y, int width, int color) {
        if (font.width(text) > width) text = font.plainSubstrByWidth(text, Math.max(1, width - font.width("…"))) + "…";
        context.centeredText(font, Component.literal(text), x, y, color);
    }
