...
public class LightLoadRule implements WildcardRule {
    ...
    public void onTick(GameContext context, int remainingTicks) {
        ...
        for (ServerPlayer player : context.getParticipants()) {
            ...
            if (weight <= 4) {
                player.addEffect(new MobEffectInstance(MobEffects.SPEED, EFFECT_TICKS, 1, false, false, true));
            } else if (weight <= 8) {
                player.addEffect(new MobEffectInstance(MobEffects.SPEED, EFFECT_TICKS, 0, false, false, true));
            } else if (weight > 12) {
                player.addEffect(new MobEffectInstance(MobEffects.SLOWNESS, EFFECT_TICKS, 0, false, false, true));
            }
        }
    }

    private int armorWeight(ServerPlayer player) {
        int weight = 0;
        for (EquipmentSlot slot : ARMOR_SLOTS) {
            weight += itemWeight(player.getItemBySlot(slot));
        }
        return weight;
    }
