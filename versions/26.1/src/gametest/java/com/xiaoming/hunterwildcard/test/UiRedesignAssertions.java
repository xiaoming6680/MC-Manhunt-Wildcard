...
final class UiRedesignAssertions {
    ...
    private static HunterWildcardConfigScreen screen(net.minecraft.client.Minecraft c){return (HunterWildcardConfigScreen)c.screen;}
    private static EditBox field(net.minecraft.client.Minecraft c){return (EditBox)c.screen.children().stream().filter(e->e instanceof EditBox).findFirst().orElseThrow();}
    private static void press(net.minecraft.client.Minecraft c,String key){String label=Component.translatable("hunterwildcard."+key).getString();Button b=(Button)c.screen.children().stream().filter(e->e instanceof Button button && button.getMessage().getString().equals(label)).findFirst().orElseThrow();check(b.active,"Button is enabled: "+key);b.onPress(new KeyEvent(257,0,0));}
    ...
    static void run(ClientGameTestContext c,TestSingleplayerContext world){
        AtomicReference<java.util.function.Consumer<net.minecraft.client.Minecraft>> preview=new AtomicReference<>();
        ...
        c.runOnClient(client->{client.options.guiScale().set(2);client.setScreen(new HunterWildcardConfigScreen());});c.waitTicks(12);
        c.runOnClient(client->{screen(client).selectPageForTesting("BASIC");
            var apply=client.screen.children().stream().filter(e->e instanceof Button b && b.getMessage().getString().equals(Component.translatable("hunterwildcard.ui.apply").getString())).map(e->(Button)e).findFirst().orElseThrow();
            check(apply.getWidth()<=76,"Apply stays compact on a wide screen");
            field(client).setValue("2:03");screen(client).onClose();client.setScreen(new HunterWildcardConfigScreen());});c.waitTicks(10);
        c.runOnClient(client->{screen(client).selectPageForTesting("BASIC");check(field(client).getValue().equals("2:03"),"Uncommitted draft survives close and reopen");press(client,"ui.apply");});c.waitTicks(15);
        ...
        c.runOnClient(client->field(client).setValue("3:00"));
        ...
        c.runOnClient(client->{check(field(client).getValue().equals("3:00"),"Broadcast does not clobber raw input");press(client,"ui.apply");});c.waitTicks(12);
        ...
        c.runOnClient(client->{press(client,"ui.undo");screen(client).selectPageForTesting("BASIC");field(client).setValue("invalid");screen(client).selectPageForTesting("GAME");screen(client).selectPageForTesting("BASIC");check(field(client).getValue().equals("invalid"),"Invalid input survives navigation");});
        ...
        c.runOnClient(client->{
            ...
            check(field(client).getValue().equals("invalid"),"Reset on another page preserves invalid raw input");
        ...
        c.runOnClient(client->{var nav=(Button)client.screen.children().stream().filter(e->e instanceof Button b&&b.getMessage().getString().equals(Component.translatable("hunterwildcard.screen.page.rules").getString())).findFirst().orElseThrow();navCursor.set(new double[]{(nav.getX()+10)*(double)client.getWindow().getScreenWidth()/client.screen.width,(nav.getY()+10)*(double)client.getWindow().getScreenHeight()/client.screen.height});});
        java.util.concurrent.atomic.AtomicReference<Object> stableNav=new java.util.concurrent.atomic.AtomicReference<>();
        c.runOnClient(client->stableNav.set(client.screen.children().getFirst()));
        c.getInput().setCursorPos(navCursor.get()[0],navCursor.get()[1]);c.waitTicks(20);
        c.runOnClient(client->check(client.screen.children().getFirst()==stableNav.get(),"Unchanged sync packets preserve menu widgets"));
        c.runOnClient(client->{check(screen(client).navigationProgressForTesting(Component.translatable("hunterwildcard.screen.page.rules").getString())>.9F,"Navigation hover remains settled across periodic refreshes");client.resizeGui();});c.waitTicks(1);
        c.runOnClient(client->check(screen(client).navigationProgressForTesting(Component.translatable("hunterwildcard.screen.page.rules").getString())>.9F,"Rebuilding widgets retains navigation animation"));
        ...
        c.runOnClient(client->{
            ...
            for(String mode:List.of("classic","dragon","survive","collect")) {
                var button=(Button)client.screen.children().stream().filter(e->e instanceof Button b&&b.getMessage().getString().equals(Component.translatable("hunterwildcard.ui.preset."+mode).getString())).findFirst().orElseThrow();
                ...
                check(client.screen.children().stream().noneMatch(e->e instanceof EditBox),"Rules overview uses readable values rather than input boxes");
                ...
                check(client.screen.children().stream().anyMatch(e->e instanceof EditBox),"Category remains editable");
                screen(client).selectPageForTesting("RULES");
            }
        ...
        c.runOnClient(client->{
            String first=Component.translatable("hunterwildcard.wildcard.backstab.name").getString();
            check(client.screen.children().stream().noneMatch(e->e instanceof Button b && b.getMessage().getString().equals(first)),"Matrix names are not clickable buttons");
            check(client.screen.children().stream().noneMatch(e->e instanceof Button b && b.getMessage().getString().equals(Component.translatable("hunterwildcard.ui.filter.short.1").getString())),"Removed enabled-state filter is absent");
            ...
            check(client.screen.children().stream().noneMatch(e->e instanceof Button b && List.of("screen.toggle.enable","screen.toggle.disable").stream().anyMatch(k->b.getMessage().getString().equals(Component.translatable("hunterwildcard."+k).getString()))),"Parameter page has no wildcard enable/disable action");
            field(client).setValue("7");
            ...
            screen(client).keyPressed(new KeyEvent(256,0,0));
            check(client.screen.children().stream().anyMatch(e->e instanceof EditBox),"Escape returns from settings to searchable matrix");
            check(client.screen.children().stream().filter(e->e instanceof EditBox).count()>=3,"Timing inputs appear directly on wildcard page");
            press(client,"ui.bulk.off");
            ...
            var offButtons=client.screen.children().stream().filter(e->e instanceof Button b && b.getMessage().getString().equals(Component.translatable("hunterwildcard.ui.bulk.off").getString())).map(e->(Button)e).toList();
            check(offButtons.size()==5,"Global and four category actions exist");
            offButtons.get(1).onPress(new KeyEvent(257,0,0));
        ...
        c.runOnClient(client->{((EditBox)client.screen.children().stream().filter(e->e instanceof EditBox f && f.getMessage().getString().equals(Component.translatable("hunterwildcard.ui.wildcard.search").getString())).findFirst().orElseThrow()).setValue("backrooms");});c.waitTicks(3);c.takeScreenshot("ui-wildcards-search");
        c.runOnClient(client->{
            AtomicReference<String> selected=new AtomicReference<>("a");
            DropdownWidget d=new DropdownWidget(client.font,0,0,100,20,"",List.of(new DropdownWidget.Option("a","A"),new DropdownWidget.Option("b","B")),"a",true,selected::set,()->{});
            d.setFocused(true);d.keyPressed(new KeyEvent(257,0,0));d.keyPressed(new KeyEvent(264,0,0));d.keyPressed(new KeyEvent(257,0,0));check(selected.get().equals("b"),"Dropdown supports keyboard selection");
        });
        c.runOnClient(client->{
        ...
        c.runOnClient(client->{
            ...
            check(client.screen.children().stream().noneMatch(e->e instanceof EditBox),"Viewer reads rule values without disabled edit fields");
        });c.takeScreenshot("ui-viewer-1280");
        world.getServer().runOnServer(HunterWildcardPackets::syncAll);c.waitTicks(4);
        c.getInput().resizeWindow(854,480);c.runOnClient(client->{client.options.guiScale().set(2);List<HunterWildcardPackets.CompassTargetEntry> entries=new ArrayList<>();for(int i=0;i<20;i++)entries.add(new HunterWildcardPackets.CompassTargetEntry(UUID.randomUUID(),"Runner_Long_Name_"+i,100+i,i%2==0,false));CompassTargetScreen.open(new HunterWildcardPackets.CompassMenuPayload(entries,true));});c.waitTicks(4);
        c.runOnClient(client->{for(var e:client.screen.children())if(e instanceof AbstractWidget w)check(w.getY()>=0&&w.getY()+w.getHeight()<=client.screen.height,"Compass controls fit viewport");});c.takeScreenshot("ui-compass-small");
        ...
        c.getInput().resizeWindow(2560,1440);c.runOnClient(client->{client.options.guiScale().set(4);client.resizeGui();});c.waitTicks(3);
        c.runOnClient(client->{var ys=new java.util.HashSet<Integer>();for(var e:client.screen.children())if(e instanceof Button b&&List.of("classic","dragon","survive","collect").stream().anyMatch(k->b.getMessage().getString().equals(Component.translatable("hunterwildcard.ui.preset."+k).getString())))ys.add(b.getY());check(ys.size()==1,"Four presets fit one row at GUI scale 4");});
        c.takeScreenshot("ui-rules-scale4");
        c.getInput().resizeWindow(1920,1080);c.runOnClient(client->{client.options.guiScale().set(2);client.resizeGui();});c.waitTicks(3);
        ...
        c.runOnClient(client->{var lines=screen(client).wrapTooltipText("回头杀\n只有背后攻击有效，伤害翻倍。",80);check(lines.size()>=3&&lines.getFirst().getString().equals("回头杀"),"Tooltip honors explicit line breaks and width wrapping");for(var line:lines)check(!line.getString().contains("\n")&&client.font.width(line)<=80,"Tooltip renders text without newline glyphs");
            var toggle=(Button)client.screen.children().stream().filter(e->e instanceof Button b&&b.getMessage().getString().equals(Component.translatable("hunterwildcard.screen.toggle.on_short").getString())).findFirst().orElseThrow();navCursor.set(new double[]{(toggle.getX()-15)*(double)client.getWindow().getScreenWidth()/client.screen.width,(toggle.getY()+7)*(double)client.getWindow().getScreenHeight()/client.screen.height});});
        ...
        c.runOnClient(client->((EditBox)client.screen.children().stream().filter(e->e instanceof EditBox f&&f.getMessage().getString().equals(Component.translatable("hunterwildcard.ui.wildcard.search").getString())).findFirst().orElseThrow()).setValue("wind_charge_brawl"));
        ...
        c.runOnClient(client->client.setScreen(new ItemPickerScreen(client.screen,id->{})));c.takeScreenshot("ui-items-zh");
        ...
        c.runOnClient(client->{preview.set(null);WildcardDrawOverlay.reset();ConfigDraft.discard();client.setScreen(null);client.options.guiScale().set(0);client.getLanguageManager().setSelected("en_us");client.options.languageCode="en_us";client.reloadResourcePacks();});waitForReload(c);
