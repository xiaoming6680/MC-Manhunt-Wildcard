...
public final class ClientGameTestContext {
    ...
    public void takeScreenshot(String name){runOnClient(c->ScreenshotRecorder.saveScreenshot(c.runDirectory,name+".png",c.getFramebuffer(),1,text->{}));}
    ...
    public final class WorldBuilder {
        ...
        public TestSingleplayerContext create(){
            runOnClient(c->CreateWorldScreen.show(c,c.currentScreen));
            waitForScreen(CreateWorldScreen.class);
            runOnClient(c->{var screen=(CreateWorldScreen)c.currentScreen; var creator=screen.getWorldCreator();creator.setWorldName("Wildcard Test "+System.currentTimeMillis());settings.accept(creator);var options=creator.getGeneratorOptionsHolder(); var dimensions=options.selectedDimensions().toConfig(options.dimensionOptionsRegistry()); var registries=options.combinedDynamicRegistries().with(net.minecraft.registry.ServerDynamicRegistryType.DIMENSIONS,dimensions.toDynamicRegistryManager()); var info=(net.minecraft.world.level.LevelInfo)TestInput.invoke(screen,"createLevelInfo",new Class<?>[]{boolean.class},false); var properties=new net.minecraft.world.level.LevelProperties(info,options.generatorOptions(),dimensions.specialWorldProperty(),com.mojang.serialization.Lifecycle.stable()); TestInput.invoke(screen,"createAndClearTempDir",new Class<?>[]{net.minecraft.registry.CombinedDynamicRegistries.class,net.minecraft.world.level.LevelProperties.class},registries,properties);});
