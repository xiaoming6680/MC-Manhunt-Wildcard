...
final class PortalSafetyAssertions {
    static void run(ServerPlayerEntity player) {
        ...
        check(player.getWorld() == fromWorld && player.getPos().equals(from), "Exit changed to fire: traversal leaves player in place");
        nether.setBlockState(feet, Blocks.AIR.getDefaultState());
        travel(context, player, nether, feet);
        check(player.getWorld() == nether && player.getY() == 80, "Valid traversal arrives in the cave below the roof");
    ...
    private static void travel(GameContext context, ServerPlayerEntity player, ServerWorld targetWorld, BlockPos pos) {
        try {
            ...
            Object entry = portalConstructor.newInstance(player.getWorld().getRegistryKey(), player.getPos());
