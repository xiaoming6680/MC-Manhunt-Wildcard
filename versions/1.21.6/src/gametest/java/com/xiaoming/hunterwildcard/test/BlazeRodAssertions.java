...
final class BlazeRodAssertions {
    ...
    private static int rods(BlazeEntity blaze, ServerPlayerEntity player, boolean playerKill) {
        try {
            var method = net.minecraft.entity.LivingEntity.class.getDeclaredMethod("dropLoot", net.minecraft.server.world.ServerWorld.class, net.minecraft.entity.damage.DamageSource.class, boolean.class);
            method.setAccessible(true);
            method.invoke(blaze, player.getWorld(), player.getDamageSources().playerAttack(player), playerKill);
        } catch (ReflectiveOperationException error) { throw new AssertionError(error); }
        var drops = player.getWorld().getEntitiesByClass(net.minecraft.entity.ItemEntity.class, blaze.getBoundingBox().expand(4), item -> item.getStack().isOf(Items.BLAZE_ROD));
