    ...
    static void run(ClientGameTestContext c, TestSingleplayerContext world) {
        ...
        world.getServer().runOnServer(server -> {
            ...
            for(int y=199;y<=203;y++)for(int z=-1;z<=1;z++)
                player.getWorld().setBlockState(new BlockPos(10,y,z),Blocks.STONE.getDefaultState());
        });
        c.waitTicks(3);
        world.getServer().runOnServer(server -> {
            var player=server.getPlayerManager().getPlayerList().getFirst();
            ...
            for(int y=199;y<=203;y++)for(int z=-1;z<=1;z++)
                player.getWorld().setBlockState(new BlockPos(10,y,z),Blocks.HAY_BLOCK.getDefaultState());
        ...
        world.getServer().runOnServer(server -> {
            ...
            for(int y=209;y<=218;y++)for(int z=60;z<=72;z++)
                player.getWorld().setBlockState(new BlockPos(60,y,z),Blocks.STONE_BRICKS.getDefaultState());
        ...
        world.getServer().runOnServer(server -> {
            var player=server.getPlayerManager().getPlayerList().getFirst();
            int y=player.getWorld().getTopY(net.minecraft.world.Heightmap.Type.MOTION_BLOCKING,0,0);
