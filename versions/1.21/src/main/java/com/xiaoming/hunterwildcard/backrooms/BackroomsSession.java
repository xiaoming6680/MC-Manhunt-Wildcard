...
public final class BackroomsSession {
    ...
    public static void initialize() {
        ...
        UseItemCallback.EVENT.register((player, world, hand) -> {
            ...
            return worldChanging ? net.minecraft.util.TypedActionResult.fail(player.getStackInHand(hand)) : net.minecraft.util.TypedActionResult.pass(player.getStackInHand(hand));
        });
    }
    ...
    private static void place(ServerPlayerEntity player, ServerWorld world, BlockPos target, float yaw, float pitch) {
        ...
        player.fallDistance = 0.0F;
        player.teleport(world, target.getX() + 0.5D, target.getY(), target.getZ() + 0.5D, Set.<PositionFlag>of(), yaw, pitch);
        player.fallDistance = 0.0F;
    }
