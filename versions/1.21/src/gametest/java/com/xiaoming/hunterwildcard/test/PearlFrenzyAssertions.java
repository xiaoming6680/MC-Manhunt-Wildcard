...
final class PearlFrenzyAssertions {
    static void run(ClientGameTestContext c, TestSingleplayerContext world, UUID playerId, UUID otherId) {
        ...
        world.getServer().runOnServer(server -> {
            ...
            var dropped = new ItemEntity(player.getServerWorld(), player.getX() + 20, player.getY(), player.getZ(), expired.get().copy());
            var ordinaryDrop = new ItemEntity(player.getServerWorld(), player.getX() + 20, player.getY(), player.getZ(), new ItemStack(Items.ENDER_PEARL, 5));
            player.getServerWorld().spawnEntity(dropped);
            player.getServerWorld().spawnEntity(ordinaryDrop);
            ...
            var result = UseItemCallback.EVENT.invoker().interact(player, player.getServerWorld(), Hand.OFF_HAND);
            check(result.getResult() == ActionResult.FAIL && player.getOffHandStack().isEmpty(), "Expired pearl cannot be thrown before cleanup tick");
        });
        c.waitTicks(25);
        world.getServer().runOnServer(server -> {
            var player = server.getPlayerManager().getPlayer(playerId);
