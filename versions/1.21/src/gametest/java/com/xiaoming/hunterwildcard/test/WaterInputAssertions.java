...
final class WaterInputAssertions {
    ...
    static void run(ClientGameTestContext c, TestSingleplayerContext world) {
        ...
        for (int index=0;index<2;index++) {
            ...
            c.runOnClient(client -> {
                ...
                s.mouseClicked(d.getX()+5,d.getY()+5,0);
                s.mouseClicked(d.getX()+5,d.getY()+d.getHeight()*2+5,0);
            });
            c.waitTicks(4);
        }
        ...
        c.runOnClient(client -> {
            ...
            check(screen.mouseClicked(field.getX()+8,field.getY()+8,0),"Mouse click reaches time input");
            check(screen.getFocused()==field&&field.isFocused(),"Clicked time input receives focus");
            check(com.xiaoming.hunterwildcard.test.harness.TestInput.keyPressed(screen,65,0,2),"Ctrl+A selects time");
            for(char ch:"2:03".toCharArray())check(screen.charTyped(ch,0),"Time accepts character "+ch);
        ...
        for (int index=0;index<4;index++) {
            ...
            c.runOnClient(client -> {
                ...
                s.mouseClicked(f.getX()+6,f.getY()+6,0);
                check(s.getFocused()==f&&f.isFocused(),"Random time input "+selected+" gets focus");
                com.xiaoming.hunterwildcard.test.harness.TestInput.keyPressed(s,65,0,2);
            });
            for(char ch:"2:03".toCharArray()) {
                c.waitTicks(3);
                c.runOnClient(client -> check(client.currentScreen.charTyped(ch,0),"Typing after redraw in random field "+selected));
            }
        }
        c.runOnClient(client -> {
        ...
        for(int index=0;index<5;index++) {
            ...
            c.runOnClient(client -> com.xiaoming.hunterwildcard.test.harness.TestInput.keyPressed(client.currentScreen,65,0,2));
        ...
        world.getServer().runOnServer(server -> {
            ...
            for(int x=-5;x<=5;x++)for(int y=195;y<=205;y++)for(int z=-5;z<=5;z++)
                p.getServerWorld().setBlockState(new BlockPos(x,y,z),Blocks.WATER.getDefaultState());
            p.networkHandler.requestTeleport(.5,200,.5,0,0);
        });
        // The fixture spans four chunks. Wait for their actual client data, even with a distant spawn.
        for(int attempt=0;attempt<600&&!c.computeOnClient(WaterInputAssertions::fixtureReady);attempt++)c.waitTicks(1);
        c.runOnClient(client -> {
            check(fixtureReady(client),"Water fixture chunks are synchronized");
            var p=client.player;
            ...
            p.getAttributeInstance(EntityAttributes.GENERIC_WATER_MOVEMENT_EFFICIENCY).setBaseValue(0);
        });
    }
    private static boolean fixtureReady(net.minecraft.client.MinecraftClient client) {
        if(client.world==null||client.player==null||Math.abs(client.player.getX()-.5)>2||Math.abs(client.player.getZ()-.5)>2||client.player.getY()<195)return false;
        for(int x=-3;x<=3;x++)for(int y=198;y<=203;y++)for(int z=-3;z<=3;z++)
            if(!client.world.getFluidState(new BlockPos(x,y,z)).isIn(FluidTags.WATER))return false;
        return true;
    }
    private static Vec3d step(PlayerEntity p,Direction d,boolean sprint,float pitch,int effects) {
        ...
        p.clearStatusEffects();p.getAttributeInstance(EntityAttributes.GENERIC_WATER_MOVEMENT_EFFICIENCY).setBaseValue(effects==1?1:0);
    ...
    private static void typeTime(net.minecraft.client.MinecraftClient client,String key,String value) {
        ...
        s.mouseClicked(f.getX()+5,f.getY()+5,0);
        check(s.getFocused()==f&&f.isFocused(),"Time click focused "+key+" active="+f.active+" visible="+f.visible+" y="+f.getY());
        com.xiaoming.hunterwildcard.test.harness.TestInput.keyPressed(s,65,0,2);
        for(char ch:value.toCharArray())check(s.charTyped(ch,0),"Time character reaches field");
