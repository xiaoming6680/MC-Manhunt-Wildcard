...
public final class WildcardHudClientGameTest implements FabricClientGameTest {
    ...
    public void runTest(ClientGameTestContext c) {
        ...
        try (TestSingleplayerContext world = c.worldBuilder()
            ...
            world.getServer().runOnServer(server -> {
                var p=player(server);var level=p.getServerWorld();
                for(int x=-1;x<=1;x++)for(int z=-1;z<=1;z++){
                    level.setBlockState(new net.minecraft.util.math.BlockPos(x,199,z),net.minecraft.block.Blocks.STONE.getDefaultState());
                    for(int y=200;y<=202;y++)level.setBlockState(new net.minecraft.util.math.BlockPos(x,y,z),net.minecraft.block.Blocks.AIR.getDefaultState());
                }
                p.setHealth(20);p.setAbsorptionAmount(0);p.timeUntilRegen=0;p.setVelocity(net.minecraft.util.math.Vec3d.ZERO);
            ...
            world.getServer().runOnServer(server -> {
                ...
                player.timeUntilRegen=0;check(player.damage(player.getServerWorld(), player.getDamageSources().generic(), 1.0F), "Damage fixture accepts the test hit");
            });
            c.waitTicks(10);
            ...
            c.waitTicks(110);
            world.getServer().runOnServer(server -> check(Math.abs(player(server).getAttributeInstance(EntityAttributes.SCALE).getValue() - 0.5D) < 0.01D, "Scale attribute halved"));
            clearChat(c);
            c.takeScreenshot("wildcard-tiny-players");
            stopWildcard(world);
            c.waitTicks(5);
            world.getServer().runOnServer(server -> check(Math.abs(player(server).getAttributeInstance(EntityAttributes.SCALE).getValue() - 1.0D) < 0.01D, "Scale restored"));
