...
final class PearlFrenzyAssertions {
    static void run(ClientGameTestContext c, TestSingleplayerContext world, UUID playerId, UUID otherId) {
        ...
        world.getServer().runOnServer(server -> {
            ...
            var dropped = new ItemEntity(player.getWorld(), player.getX() + 20, player.getY(), player.getZ(), expired.get().copy());
            var ordinaryDrop = new ItemEntity(player.getWorld(), player.getX() + 20, player.getY(), player.getZ(), new ItemStack(Items.ENDER_PEARL, 5));
            player.getWorld().spawnEntity(dropped);
            player.getWorld().spawnEntity(ordinaryDrop);
            ...
            var result = UseItemCallback.EVENT.invoker().interact(player, player.getWorld(), Hand.OFF_HAND);
