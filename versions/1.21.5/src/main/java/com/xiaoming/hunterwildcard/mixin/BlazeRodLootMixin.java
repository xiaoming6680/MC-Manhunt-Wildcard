...
public class BlazeRodLootMixin {
    ...
    private void hunterwildcard$blazeRodChance(LootTable table, LootWorldContext parameters, long seed, Consumer<ItemStack> consumer, Operation<Void> original, ServerWorld world, DamageSource source, boolean causedByPlayer) {
        ...
        if (!((Object) this instanceof BlazeEntity blaze) || !causedByPlayer || !config.blazeRodChanceEnabled
                || !blaze.getLootTableKey().orElseThrow().getValue().equals(Identifier.ofVanilla("entities/blaze"))) {
            original.call(table, parameters, seed, consumer);
            return;
        }
        var drops = new ArrayList<ItemStack>();
        original.call(table, parameters, seed, (Consumer<ItemStack>) drops::add);
