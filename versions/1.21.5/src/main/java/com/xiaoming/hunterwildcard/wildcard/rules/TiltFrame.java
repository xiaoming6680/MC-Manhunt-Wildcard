...
public final class TiltFrame {
    ...
    public static void look(Entity entity,float yaw,float pitch) {
        ...
        entity.lastYaw=entity.getYaw();entity.lastPitch=entity.getPitch();
        if(entity instanceof net.minecraft.entity.LivingEntity living) {living.setHeadYaw(entity.getYaw());living.lastHeadYaw=entity.getYaw();}
    }
    ...
    public static java.util.Optional<BlockPos> supportingBlock(Entity entity) {
        ...
        return entity.getWorld().findSupportingBlockPos(entity,feet);
    }
    public static boolean grounded(Entity entity) {
        Vec3d probe=down(entity).multiply(.05);
        return Entity.adjustMovementForCollisions(entity,probe,entity.getBoundingBox(),entity.getWorld(),List.of())
    ...
    public static void change(Entity entity, Direction direction) {
        ...
        if (!entity.getWorld().isSpaceEmpty(entity,candidate)) {
            boolean found=false;
            for(double distance=.1;distance<=2.01&&!found;distance+=.1) {
                for(Direction escape:Direction.values()) {
                    ...
                    if (entity.getWorld().isSpaceEmpty(entity,candidate.offset(offset))
                            && entity.getWorld().isSpaceEmpty(entity,oldBox.stretch(offset).contract(.001))) {
            ...
            if(!found && direction==Direction.DOWN) {
                ...
                if(entity.getWorld().isSpaceEmpty(entity,box(entity,small,smallFeet))) {
    ...
    public static Vec3d collide(Entity entity, Vec3d movement) {
        ...
        entity.getWorld().getCollisions(entity,search).forEach(worldShapes::add);
        if(entity.getWorld().getWorldBorder().canCollide(entity,search))worldShapes.add(entity.getWorld().getWorldBorder().asVoxelShape());
