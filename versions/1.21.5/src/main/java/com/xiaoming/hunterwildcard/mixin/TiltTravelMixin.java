...
public abstract class TiltTravelMixin {
    ...
    private void hunterwildcard$travel(Vec3d input,CallbackInfo ci) {
        ...
        float slipperiness=self.isOnGround()?self.getWorld().getBlockState(TiltFrame.support(self)).getBlock().getSlipperiness():1;
        ...
        if(levitation!=null)vertical+=(.05*(levitation.getAmplifier()+1)-vertical)*.2;
            ...
            double gravity=self.getAttributeValue(EntityAttributes.GRAVITY);
    ...
    private void hunterwildcard$fluid(Vec3d input,CallbackInfo ci) {
        ...
        float friction=self.isSprinting()?.9F:getBaseWaterMovementSpeedMultiplier();
        float speed=.02F;
        if(water) {
            float efficiency=(float)self.getAttributeValue(EntityAttributes.WATER_MOVEMENT_EFFICIENCY);
