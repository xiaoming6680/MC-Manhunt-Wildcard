...
final class WorldTiltAssertions {
    ...
    static void run(ClientGameTestContext c, TestSingleplayerContext world) {
        ...
        world.getServer().runOnServer(server -> {
            ...
            var dimension=player.getEntityWorld();
        ...
        world.getServer().runOnServer(server -> {
            ...
            for(int y=199;y<=203;y++)for(int z=-1;z<=1;z++)
                player.getEntityWorld().setBlockState(new BlockPos(10,y,z),Blocks.STONE.getDefaultState());
        });
        c.waitTicks(3);
        world.getServer().runOnServer(server -> {
            var player=server.getPlayerManager().getPlayerList().getFirst();
            ...
            for(int y=199;y<=203;y++)for(int z=-1;z<=1;z++)
                player.getEntityWorld().setBlockState(new BlockPos(10,y,z),Blocks.HAY_BLOCK.getDefaultState());
        ...
        world.getServer().runOnServer(server -> {
            ...
            for(int y=209;y<=218;y++)for(int z=60;z<=72;z++)
                player.getEntityWorld().setBlockState(new BlockPos(60,y,z),Blocks.STONE_BRICKS.getDefaultState());
        ...
        c.runOnClient(client -> {
            ...
            check(client.player.isOnGround(),"Real client remains grounded on wall after twenty movement ticks: pos="+client.player.getEntityPos()+", velocity="+client.player.getVelocity()+", pose="+client.player.getPose()+", support="+client.world.getBlockState(com.xiaoming.hunterwildcard.wildcard.rules.TiltFrame.support(client.player))+", flying="+client.player.getAbilities().flying);
            Camera camera=new Camera();camera.update(client.world,client.player,false,false,1);
            check(camera.getCameraPos().distanceTo(client.player.getEyePos())<.001,"Rendered camera origin matches interaction ray origin");
            try {
        ...
        c.runOnClient(client->check(client.player.getX()<59.8,"Real client jump moves away from gravity floor: pos="+client.player.getEntityPos()+", velocity="+client.player.getVelocity()+", ground="+client.player.isOnGround()));
        c.waitTicks(20);
        ...
        world.getServer().runOnServer(server->{
            var player=server.getPlayerManager().getPlayerList().getFirst();
            check(player.isOnGround()&&player.getHealth()<20,"Actual synchronized sideways fall lands and applies native damage: health="+player.getHealth()+", pos="+player.getEntityPos());
        });
        ...
        world.getServer().runOnServer(server -> {
            var player=server.getPlayerManager().getPlayerList().getFirst();
            int y=player.getEntityWorld().getTopY(net.minecraft.world.Heightmap.Type.MOTION_BLOCKING,0,0);
