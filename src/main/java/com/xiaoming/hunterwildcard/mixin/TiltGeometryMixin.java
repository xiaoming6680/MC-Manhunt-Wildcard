...
public abstract class TiltGeometryMixin {
    ...
    private void hunterwildcard$box(Vec3d pos,CallbackInfoReturnable<Box> cir) {
        Entity self=(Entity)(Object)this;
    ...
    private void hunterwildcard$syncBox(TrackedData<?> data,CallbackInfo ci) {
        ...
        if(self instanceof com.xiaoming.hunterwildcard.wildcard.rules.TiltTrackedGravity)
            self.setBoundingBox(TiltFrame.box(self,self.getDimensions(self.getPose()),self.getEntityPos()));
    }
    @Inject(method="calculateDimensions",at=@At("TAIL"))
    private void hunterwildcard$resizeBox(CallbackInfo ci) {
        Entity self=(Entity)(Object)this;
        if(TiltFrame.active(self)) self.setBoundingBox(TiltFrame.box(self,self.getDimensions(self.getPose()),self.getEntityPos()));
    }
    @Inject(method="getEyePos",at=@At("HEAD"),cancellable=true)
    private void hunterwildcard$eyes(CallbackInfoReturnable<Vec3d> cir) {
        Entity self=(Entity)(Object)this;
        if(TiltFrame.active(self)) cir.setReturnValue(TiltFrame.eye(self,self.getEntityPos()));
    }
    @Inject(method="getEyeY",at=@At("HEAD"),cancellable=true)
    private void hunterwildcard$eyeY(CallbackInfoReturnable<Double> cir) {
        Entity self=(Entity)(Object)this;
        if(TiltFrame.active(self))cir.setReturnValue(TiltFrame.eye(self,self.getEntityPos()).y);
    }
    ...
    private void hunterwildcard$camera(float tick,CallbackInfoReturnable<Vec3d> cir) {
        Entity self=(Entity)(Object)this;
        if(TiltFrame.active(self)) cir.setReturnValue(TiltFrame.eye(self,new Vec3d(MathHelper.lerp(tick,self.lastX,self.getX()),MathHelper.lerp(tick,self.lastY,self.getY()),MathHelper.lerp(tick,self.lastZ,self.getZ()))));
    }
    ...
    private void hunterwildcard$start(MovementType type,Vec3d movement,CallbackInfo ci) {
        Entity self=(Entity)(Object)this;
        hunterwildcard$moveStart=TiltFrame.active(self)?self.getEntityPos():null;
    }
    ...
    private void hunterwildcard$ground(MovementType type,Vec3d movement,CallbackInfo ci) {
        ...
        Vec3d actual=TiltFrame.local(TiltFrame.direction(self),self.getEntityPos().subtract(hunterwildcard$moveStart));
