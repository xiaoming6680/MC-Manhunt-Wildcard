...
public final class CombatFeed {
    ...
    public static void kill(String hunter,String runner,int current,int target,boolean environment){
        ...
        sm.play(PositionedSoundInstance.ui(SoundEvents.ENTITY_ARROW_HIT_PLAYER,environment?0.75F:1.0F));
    }
    public static void notice(String title,String a,String b,String style){
        if ("kill".equals(style) && a != null && a.startsWith(key("hud.feedback.versus")+HunterWildcardText.SPEC_SEPARATOR)) {
            ...
            if(parts.length==3){Entry e=new Entry(title,parts[1],parts[2],"kill",90,0,0);e.status=b;e.environment=title.contains("env_kill");enqueue(e);MinecraftClient.getInstance().getSoundManager().play(PositionedSoundInstance.ui(SoundEvents.ENTITY_ARROW_HIT_PLAYER,e.environment?0.75F:1F));return;}
        }
