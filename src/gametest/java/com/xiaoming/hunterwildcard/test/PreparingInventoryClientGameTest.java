            ...
            for (PlayerRole role : PlayerRole.values()) {
                for (boolean keep : new boolean[]{true, false}) {
                    world.getServer().runOnServer(server -> {
                        ...
                        server.getOverworld().getGameRules().setValue(GameRules.KEEP_INVENTORY, true, server);
                        manager.start(server.getCommandSource());
                        check(manager.getState() == GameState.PREPARING, "Test must remain in preparation");
                        check(!server.getOverworld().getGameRules().getValue(GameRules.KEEP_INVENTORY), "Round takes over vanilla inventory rule");
                    ...
                    world.getServer().runOnServer(server -> {
                        ...
                        var drops = player.getEntityWorld().getEntitiesByClass(ItemEntity.class,
                        ...
                        player.getEntityWorld().getEntitiesByClass(ItemEntity.class, player.getBoundingBox().expand(5), e -> true)
